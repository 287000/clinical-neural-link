...
function renderStudentAssessmentPortal() {
    ...
    const currentCourseName = (window.currentSelection && window.currentSelection.course) || 
                              localStorage.getItem('active_course') || 
                              "Anatomy";
    
    // Safely match program strings to lowercase format used by compileAndSaveQuizConfiguration()
    const finalProgram = (window.currentSelection?.program || localStorage.getItem('active_program') || 'mbchb').toLowerCase();
    const finalYear = window.currentSelection?.year || localStorage.getItem('active_year') || '2';
    ...
    // 🎨 UI CONTAINER MARKUP — Interactive Slot Cards
    const listMarkup = `
        <div class="flex flex-col space-y-3 w-full">
            ${assessmentSlots.map(slot => `
                <div id="slot-card-${slot.id}" 
                    class="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-[#050b18]/85 border ${slot.isLive ? 'border-purple-500/30 bg-purple-950/5' : 'border-slate-800/40'} rounded-xl p-3.5 sm:p-4 hover:border-purple-500/40 shadow-sm hover:shadow-[0_0_20px_-5px_rgba(147,51,234,0.1)] transition-all group w-full gap-3 sm:gap-0">
                    
                    <div class="flex items-center space-x-3.5 min-w-0 w-full sm:w-auto">
                        <div class="p-2 sm:p-2.5 ${slot.isLive ? 'bg-purple-500/10 text-purple-400' : 'bg-amber-500/10 text-amber-500'} rounded-lg transition-colors shrink-0">
                            <i data-lucide="${slot.isLive ? 'file-text' : 'folder'}" class="w-4 h-4 sm:w-5 sm:h-5"></i>
                        </div>
                        <div class="min-w-0">
                            <h3 class="text-xs font-black text-slate-200 tracking-wide uppercase group-hover:text-white transition-colors truncate">${slot.title}</h3>
                    ...
                    <button onclick="window.startActiveQuizEngine('${finalProgram}', '${finalYear}', '${slot.id}')" 
                        class="w-full sm:w-auto text-center px-4 py-2 sm:py-1.5 bg-[#0a1224] hover:bg-purple-600 text-slate-300 hover:text-white text-[10px] font-black uppercase tracking-wider rounded-lg border border-slate-800 hover:border-purple-500 transition-all transform active:scale-98 shadow-sm cursor-pointer">
                        Configure & Launch
                    </button>
                </div>
            `).join('')}
    ...
    // Render Portal Workspace Layout
    contentArea.innerHTML = `
        <div class="w-full h-full max-w-5xl mx-auto flex flex-col animate-in fade-in duration-300 pt-2 sm:pt-4 px-2 sm:px-4">
            ...
            <div class="flex items-center justify-between mb-6 pb-4 border-b border-slate-800/60">
                ...
                <div class="hidden sm:flex items-center space-x-2 text-[10px] font-mono text-slate-500 uppercase tracking-widest">
                    ...
                    <span>Year 0${finalYear}</span>
                    <i data-lucide="chevron-right" class="w-3 h-3 text-slate-600"></i>
                    <span class="truncate max-w-[120px]">${currentCourseName}</span>
                    <i data-lucide="chevron-right" class="w-3 h-3 text-slate-600"></i>
                    <span class="text-purple-400 font-bold">Assessments</span>
                </div>
            </div>

            <!-- SECTION HEADER -->
            <div class="mb-6 sm:mb-8">
                <h2 class="text-lg sm:text-2xl font-black text-white uppercase tracking-wider">${currentCourseName} &mdash; Assessments</h2>
                <p class="text-[9px] sm:text-[10px] text-blue-400 font-bold uppercase tracking-widest mt-1">Select active file bank to initialize interactive clinical session</p>
            </div>
